      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex-shrink-0">
            <span 
              className="text-xl font-bold text-amber-600 cursor-pointer hover:text-amber-700 transition-colors"
                ...
                <button
                  ...
                  className={`px-3 py-2 text-sm font-medium transition-all duration-300 hover:text-amber-600 ${
                    activeSection === item.id
                      ...
                      : 'text-slate-700 hover:border-b-2 hover:border-amber-600'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 bg-white rounded-lg mt-2 border border-slate-200 shadow-lg">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => handleNavClick(item.id)}
